...
main_dict = csv.to_dict(orient='records')
current_card = {}
...
def get_next_card():
    ...
    current_card = random.choice(main_dict)
...
def card_known():
    main_dict.remove(current_card)
    new_data = pd.DataFrame(main_dict)
...
known_button = Button(image=check_image, highlightthickness=0, command=card_known)
